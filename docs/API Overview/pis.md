...
## Base URL
The base URL for all PIS APIs is: `https://rs1.openbanking.ifastgb.com/open-banking/v3.1/pisp/**`

## Supported Payment Types
The iFast API currently only supports:
- Domestic Payments

The iFast API __does not__ support:
...
- There is no MAX `InstructedAmount/Amount` mandated by iFast API.
-  iFast suggest PISP notify the PSU that the same limits apply as in their iFast app. It is possible from time to time that `domestic-payment-consents` is authorised, but the payment initiation fails due to account limits.
